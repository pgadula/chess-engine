...
use chess_core::bitboard::{GameState, TEMP_VALID_MOVE_SIZE};
use chess_core::fen::FenParser;
use chess_core::types::PieceMove;
...
fn stock_fish_perft(fen: &str, depth: usize) -> Result<StockfishOutput, Error> {
    ...
    let stockfish_path = "/Users/pgadula/Programming/stockfish/stockfish-macos-m1-apple-silicon";
...
fn python_get_moves(fen: &str) -> Option<Vec<String>> {
    ...
    let result = Command::new("python3")
        .args([
            "/Users/pgadula/Programming/python_position_checker/move_generator.py",
            fen,
            depth,
        ])
        .output();
...
fn make_move(fen: &str, move_uci: &str) -> Option<String> {
    let result = Command::new("python3")
        .args(["/Users/pgadula/Programming/python_position_checker/apply_move.py", fen, move_uci])
        .output();
...
impl CalculationObject {
    ...
    pub fn get_total_nodes(&mut self, game: &mut GameState, depth: usize) -> usize {
        ...
        let valid_moves:&mut [PieceMove; 512] = &mut [PieceMove::default(); TEMP_VALID_MOVE_SIZE];

        let count =  game.fill_valid_moves(valid_moves);
        let mut nodes = 0;
        let fen = GameState::serialize(&game);
        // log_diff(&fen, &valid_attacks);

        for valid_move in valid_moves[..count].iter() {
        ...
        log_diff(&fen, &valid_moves[..count].to_vec());
