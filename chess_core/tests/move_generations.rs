...
fn check_position() {
    for case in TEST_CASES {
        let mut game = GameState::deserialize(&case.fen);

        let result = get_total_nodes(&mut game, case.depth as usize);
        assert_eq!(
            case.nodes, result,
            "we are testing move generation with fen  {} with {} and {}",
            case.fen, case.nodes, result
        )
    }
}
pub fn get_total_nodes(game: &mut GameState, depth: usize) -> usize {
    if depth == 0 {
        return 1;
    }
    ...
    for valid_move in valid_attacks.iter() {
        ...
        let move_nodes = get_total_nodes(&mut clone_game.clone(), depth - 1);
        nodes += move_nodes;
    }
...
pub const TRICKY_CASES: &[TestPosition] = &[
    TestPosition {
        depth: 1,
        nodes: 45,
