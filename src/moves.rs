...
use crate::{game::Game, types::{FileRank, Move, FILE_RANK, RANK_3, RANK_6}, utility::print_chessboard_from_u64};

pub fn get_pawn_moves(game: &Game)->Vec<Move>{
    let mut moves:Vec<Move> = Vec::new();
    let mut pawns =  if game.w_turn { game.w_pawn } else { game.b_pawn };
